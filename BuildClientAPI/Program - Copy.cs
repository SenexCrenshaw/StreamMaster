...
namespace BuildClientAPI
{
    internal class Program2
    {
        ...
        internal static void ScanForSMAPI()
        {
            ...
            try
            {
                    ...
                    foreach (MethodInfo method in type.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static))
                    {
                        ...
                        if (smapiAttribute != null)
                        {
                            ...
                            if (method.Name == "DeleteSMChannels")
                            {
                                ...
                                string Parameters = ParameterConverter2.ParamsToCSharp(method);
                                string TsParameters = ParameterConverter2.CSharpParamToTS(method); ;
                            }

                            string ps = ParameterConverter2.ParamsToCSharp(method);
                            string tsps = ParameterConverter2.CSharpParamToTS(method); ;
                            ...
                            MethodDetails methodDetails = new()
                            {
                                ...
                                TsParameterTypes = string.Join(", ", method.GetParameters().Select(p => ParameterConverter2.MapCSharpTypeToTypeScript(ParameterConverter2.GetTypeFullNameForParameter(p.ParameterType))))
                            };
                ...
                foreach (KeyValuePair<string, List<MethodDetails>> kvp in methodsByNamespace)
                {
                    ...
                    string entityName = ParameterConverter2.ExtractInnermostType(methods.First().ReturnType);
                    ...
                    if (writeFiles)
                    {
                        ...
                        TypeScriptCommandGenerator2.GenerateFile(namespaceName, entityName, methods, tsCommandFilePath);
                        ...
                        if (pagedMethods.Count > 0)
                        {
                            ...
                            TypeScriptSliceGenerator2.GenerateFile(namespaceName, entityName, pagedMethods, tsSliceFilePath);


                            string tsHookFilePath = Path.Combine(SMAPIFileNamePrefix, namespaceName, $"use{namespaceName}.ts");
                            TypeScriptHookGenerator2.GenerateFile(namespaceName, entityName, methods, tsHookFilePath);
