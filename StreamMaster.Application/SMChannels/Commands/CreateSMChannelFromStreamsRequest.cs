...
public record CreateSMChannelFromStreamsRequest(List<string> StreamIds, int? StreamGroupId) : IRequest<APIResponse>;
...
internal class CreateSMChannelFromStreamsRequestHandler(IRepositoryWrapper Repository, IDataRefreshService dataRefreshService) : IRequestHandler<CreateSMChannelFromStreamsRequest, APIResponse>
{
    ...
    {
        APIResponse res = await Repository.SMChannel.CreateSMChannelFromStreams(request.StreamIds, request.StreamGroupId);
        if (res.IsError)
        {
