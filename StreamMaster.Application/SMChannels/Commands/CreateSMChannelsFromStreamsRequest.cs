...
public record CreateSMChannelsFromStreamsRequest(List<string> StreamIds, int? M3UFileId, int? StreamGroupId = null, bool? IsCustomPlayList = false, bool? forced = false) : IRequest<APIResponse>;
...
internal class CreateSMChannelsFromStreamsRequestHandler(IRepositoryWrapper Repository, IDataRefreshService dataRefreshService) : IRequestHandler<CreateSMChannelsFromStreamsRequest, APIResponse>
{
    ...
    {
        APIResponse res = await Repository.SMChannel.CreateSMChannelsFromStreams(request.StreamIds, AddToStreamGroupId: request.StreamGroupId, M3UFileId: request.M3UFileId, IsCustomPlayList: request.IsCustomPlayList, forced: request.forced);
        if (res.IsError)
        {
