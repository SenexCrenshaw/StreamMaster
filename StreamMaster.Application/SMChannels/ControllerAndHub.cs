...
namespace StreamMaster.Application.SMChannels.Controllers
{
    ...
    public partial class SMChannelsController(ILogger<SMChannelsController> _logger) : ApiControllerBase, ISMChannelsController
    {
        ...
        {
            var ret = await APIStatsLogger.DebugAPI(Sender.Send(new GetPagedSMChannelsRequest(Parameters))).ConfigureAwait(false);
            return ret?? new();
        }
        [HttpGet]
        [Route("[action]")]
        public async Task<ActionResult<string>> GetSMChannelUniqueName([FromQuery] GetSMChannelUniqueNameRequest request)
        {
            try
            {
            var ret = await APIStatsLogger.DebugAPI(Sender.Send(request)).ConfigureAwait(false);
             return ret.IsError ? Problem(detail: "An unexpected error occurred retrieving GetSMChannelUniqueName.", statusCode: 500) : Ok(ret.Data?? string.Empty);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "An unexpected error occurred while processing the request to get GetSMChannelUniqueName.");
...
namespace StreamMaster.Application.Hubs
{
    public partial class StreamMasterHub : ISMChannelsHub
    {
        ...
        public async Task<string> GetSMChannelUniqueName(GetSMChannelUniqueNameRequest request)
        {
             var ret = await APIStatsLogger.DebugAPI(Sender.Send(request)).ConfigureAwait(false);
