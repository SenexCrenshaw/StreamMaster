...
public record UpdateStreamGroupRequest(int StreamGroupId, bool? AutoSetChannelNumbers, string? NewName, List<string>? StreamGroupProfiles, bool? IgnoreExistingChannelNumbers, int? StartingChannelNumber)
    ...
    : IRequestHandler<UpdateStreamGroupRequest, APIResponse>
{
    public async Task<APIResponse> Handle(UpdateStreamGroupRequest request, CancellationToken cancellationToken)
    {
        ...
        StreamGroupDto? streamGroup = await Repository.StreamGroup.UpdateStreamGroup(request.StreamGroupId, request.NewName, request.AutoSetChannelNumbers, request.IgnoreExistingChannelNumbers, request.StartingChannelNumber);
