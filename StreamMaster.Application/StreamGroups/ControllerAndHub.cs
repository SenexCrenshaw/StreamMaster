...
namespace StreamMaster.Application.StreamGroups.Controllers
{
    ...
    public partial class StreamGroupsController(ILogger<StreamGroupsController> _logger) : ApiControllerBase, IStreamGroupsController
    {
        ...
        {
            var ret = await APIStatsLogger.DebugAPI(Sender.Send(new GetPagedStreamGroupsRequest(Parameters))).ConfigureAwait(false);
            return ret?? new();
        }
        [HttpGet]
        [Route("[action]")]
        public async Task<ActionResult<List<StreamGroupProfile>>> GetStreamGroupProfiles()
        {
            try
            {
            var ret = await APIStatsLogger.DebugAPI(Sender.Send(new GetStreamGroupProfilesRequest())).ConfigureAwait(false);
        ...
        public async Task<ActionResult<List<StreamGroupDto>>> GetStreamGroups()
        {
            try
            {
            var ret = await APIStatsLogger.DebugAPI(Sender.Send(new GetStreamGroupsRequest())).ConfigureAwait(false);
