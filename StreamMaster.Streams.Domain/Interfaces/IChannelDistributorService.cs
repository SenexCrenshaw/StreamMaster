...
namespace StreamMaster.Streams.Domain.Interfaces
{
    public interface IChannelDistributorService
    {
        ...
        Task<IChannelDistributor?> GetOrCreateSourceChannelDistributorAsync(string channelName, SMStreamInfo smStreamInfo, CancellationToken cancellationToken);
        bool StopAndUnRegister(string key);
        List<IChannelDistributor> GetChannelDistributors();
        Task<IChannelDistributor?> CreateChannelDistributorFromSMChannelDtoAsync(SMChannelDto smChannel, IChannelStatus channelStatus, CancellationToken cancellationToken);
    }
}