...
public class CommandStream(IHTTPStream HTTPStream) : ICommandStream
{
    public async Task<(Stream? stream, int processId, ProxyStreamError? error)> HandleStream(SMStreamInfo SMStreamInfo, string clientUserAgent, CancellationToken cancellationToken)
    {
        ...
        //{
        //    //if (profileSettings.CurrentValue.CommandProfileDict.TryGetValue("FFMPEG", out CommandProfile? ffmpegProfile))
        //    //{
        ...
        //    //}
        //    logger.LogInformation("Stream URL has HLS content, using FFMpeg for streaming: {StreamUrl} {streamName}", smStreamDto.Url, smStreamDto.Name);
