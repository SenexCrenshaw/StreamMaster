public class FileSaver
{
    ...
    public async Task SaveVideoWithRevisionsAsync(byte[] videoMemory, string baseFilePath)
    {
        ...
        // Save the new file
        using (FileStream fs = new FileStream(newFilePath, FileMode.Create, FileAccess.Write, FileShare.None, 4096, true))
        {
    ...
    private void ManageOldRevisions(string directory, string baseFileName, string extension)
    {
        ...
        var files = Directory.GetFiles(directory, $"{baseFileName}.*{extension}")
                             .Where(f => Path.GetFileNameWithoutExtension(f).StartsWith($"{baseFileName}.") &&
                                         int.TryParse(Path.GetFileNameWithoutExtension(f).Substring(baseFileName.Length + 1), out _))
                             .OrderBy(f => f)
                             .ToList();
