...
public class MultiViewPlayListStream(ILogger<MultiViewPlayListStream> logger, ICacheManager cacheManager, ICommandExecutor commandExecutor) : IMultiViewPlayListStream
{
    public async Task<(Stream? stream, int processId, ProxyStreamError? error)> HandleStream(IChannelBroadcaster channelBroadcaster, CancellationToken cancellationToken)
    {
        ...
        string filter_complex = "[0:v]scale=960:540[top_left]; [1:v]scale=960:540[top_right]; [2:v]scale=960:540[bottom_left];[3:v]scale=960:540[bottom_right];[top_left][top_right]hstack=inputs=2[top]; [bottom_left][bottom_right]hstack=inputs=2[bottom]; [top][bottom]vstack=inputs=2[out]";
