...
public class GetSchedulesHandler(ISDService sdService, ILogger<GetSchedules> logger, IRepositoryWrapper repository, IMapper mapper, ISettingsService settingsService, IPublisher publisher, ISender sender, IHubContext<StreamMasterHub, IStreamMasterHub> hubContext, IMemoryCache memoryCache)
: BaseMediatorRequestHandler(logger, repository, mapper, settingsService, publisher, sender, hubContext, memoryCache), IRequestHandler<GetSchedules, List<Schedule>>
{
    public async Task<List<Schedule>> Handle(GetSchedules request, CancellationToken cancellationToken)
    {
        ...
        List<Schedule>? ret = await sdService.GetSchedules(setting.SDStationIds.Select(a => a.StationId).ToList(), cancellationToken).ConfigureAwait(false);
