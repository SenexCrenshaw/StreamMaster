...
const VideoPlayerDialog = (props: VideoPlayerDialogProps) => {
  ...
  const [videoStreamId, setVideoStreamId] = useLocalStorage<string>(
    '',
    'video-player-videoStreamId',
  )
  const [epgMoused, setEpgMoused] = React.useState<boolean>(true)
  const [src, setSrc] = React.useState<string>(
    'http://shine.jserv.me/live/Senex/CNG4s5XGZZ/50b31b45-30d3-4ee3-b6ea-c7e54b7bc021.ts',
  )
  ...
  const getEpg = React.useCallback(
    (channel: string): EpgProgram | undefined => {
      const epg = epgForGuide.data?.programs?.find(
        (p) =>
          p.channelUuid === channel &&
          p.since !== undefined &&
          p.till !== undefined &&
          new Date(p.since) <= new Date() &&
          new Date(p.till) >= new Date(),
      )
  ...
  React.useEffect(() => {
    if (videoStreamId !== '') {
      const videoStream = videoStreamsQuery.data?.data?.find(
        (v) => v.id === videoStreamId,
      )
