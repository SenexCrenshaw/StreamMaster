...
import {
  ...
  type ColumnMeta,
  type DataSelectorSelectionMode,
...
const DataSelector = <T extends DataTableValue>(
  props: DataSelectorProps<T>,
) => {
  const { state, setters } = useDataSelectorState<T>(
    props.id,
  ...
  useEffect(() => {
    ...
    if (
      ...
      state.sortField !== props.defaultSortField
    ) {
  ...
  useEffect(() => {
    ...
    if (props.dataSource) {
      if (
        !state.dataSource ||
        (state.dataSource &&
          !areArraysEqual(props.dataSource, state.dataSource))
      ) {
        if (!props.reorderable) {
          setters.setDataSource(props.dataSource)
        } else {
          setters.setDataSource(
            [...props.dataSource].sort((a, b) => a.rank - b.rank),
          )
        }
    ...
    if (Array.isArray(data)) {
      if (
        !state.dataSource ||
        (state.dataSource && !areArraysEqual(data, state.dataSource))
      ) {
  ...
  const rowClass = useCallback(
    (changed: DataTableRowData<T[]>) => {
      ...
      if (
        props.videoStreamIdsIsReadOnly !== undefined &&
        props.videoStreamIdsIsReadOnly.length > 0
      ) {
        const isReadOnly = props.videoStreamIdsIsReadOnly.find(
          (vs) => vs === getRecord(changed as T, 'id'),
        )
  ...
  const getAlignHeader = useCallback(
    (
      ...
      return (
        <span className="vertical-align-middle ml-2 font-bold line-height-3">
          ...
          <Button
            ...
            icon={`pi pi-eye ${
              row.isHidden ? 'text-red-500' : 'text-green-500'
            }`}
    ...
    return (
      <div className="text-xs text-white text-500">
        <BanButton
          ...
          tooltip={`Clear ${
            (state.selectSelectedItems || []).length
          } Selections`}
        />
      </div>
    )
  ...
  const onSort = (event: DataTableStateEvent) => {
    ...
    const sortOrder = [1, 0, -1].includes(event.sortOrder ?? 1)
      ? event.sortOrder
      : 0
    ...
    let matchingColumn = props.columns.find(
      (column) => column.field === event.sortField,
    )

    // If not found, try finding by header, case-insensitively.
    if (!matchingColumn) {
      matchingColumn = props.columns.find(
        (column) =>
          column.header?.toLocaleLowerCase() ===
          event.sortField.toLocaleLowerCase(),
      )
    }
  ...
  return (
    <div className="dataselector flex w-full min-w-full  justify-content-start align-items-center">
      <div
        ...
        } min-h-full w-full surface-overlay`}
      >
        <DataTable
          cellSelection={false}
          dataKey="id"
          ...
          expandableRowGroups={
            props.groupRowsBy !== undefined && props.groupRowsBy !== ''
          }
          ...
          filterDisplay="row"
          filters={
            isEmptyObject(state.filters)
              ? getEmptyFilter(props.columns, state.showHidden)
              : state.filters
          }
          first={
            state.pagedInformation ? state.pagedInformation.first : state.first
          }
          ...
          totalRecords={
            state.pagedInformation
              ? state.pagedInformation.totalItemCount
              : undefined
          }
...
type QueryFilterProps<T> = BaseDataSelectorProps<T> & {
  dataSource?: never
  queryFilter: (
    filters: GetApiArg | typeof skipToken,
  ) => ReturnType<QueryHook<PagedResponseDto<T> | T[]>>
}
