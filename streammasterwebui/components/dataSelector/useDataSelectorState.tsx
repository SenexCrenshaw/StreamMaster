...
const useDataSelectorState = <T extends DataTableValue>(
  ...
  const { selectSelectedItems, setSelectSelectedItems } =
    useSelectedItems<T>(selectedItemsKey)
  const { showHidden } = useShowHidden(id)

  const [rowClick, setRowClick] = useLocalStorage<boolean>(
    false,
    id + '-rowClick',
  )

  const [pagedInformation, setPagedInformation] =
    useState<PagedTableInformation>()
  const [prevDataSource, setPrevDataSource] = useState<T[] | undefined>(
    undefined,
  )
  ...
  const [videoStreamIsReadOnlys, setVideoStreamIsReadOnlys] = useState<
    VideoStreamIsReadOnly[]
  >([])
