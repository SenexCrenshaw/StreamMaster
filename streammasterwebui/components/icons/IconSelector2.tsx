...
const IconSelector2 = ({ enableEditMode = true, value, disabled, editable = true, onChange, useDefault }: IconSelector2Properties) => {
  ...
  const panelTemplate = (option: any) => {
    return (
      <div className="iconselector2 flex flex-row grid col-12 m-0 p-0 justify-content-between align-items-center">
  ...
  const className = classNames('align-contents-center p-0 m-0 max-w-full w-full iconselector2', {
    'p-disabled': disabled
  });
  ...
  if (!enableEditMode) {
    return <div className="flex w-full h-full justify-content-center align-items-center p-0 m-0 iconselector2">{input ?? 'Dummy'}</div>;
  }

  if (loading) {
    return (
      <div className="iconselector2 m-0 p-0">
        <ProgressSpinner />
      </div>
    );
  }
  ...
  return (
    <div className="flex align-contents-center w-full min-w-full iconselector2">
      <Dropdown
        className={className}
        disabled={loading}
...
IconSelector2.displayName = 'IconSelector2';

export default React.memo(IconSelector2);
