...
const EPGSelector: React.FC<Partial<EPGSelectorProps>> = ({ enableEditMode = true, onChange, ...restProps }) => {
  ...
  if (!enableEditMode) {
    return <div className="flex h-full justify-content-center align-items-center p-0 m-0">{restProps.value ?? 'Dummy'}</div>;
  }
