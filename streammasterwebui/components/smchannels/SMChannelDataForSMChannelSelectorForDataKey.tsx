...
import { M3UFileDto, SMChannelDto } from '@lib/smAPI/smapiTypes';
import { ColumnFilterElementTemplateOptions } from 'primereact/column';
import { MultiSelect, MultiSelectChangeEvent } from 'primereact/multiselect';
import { ReactNode, memo, useCallback, useMemo, useState } from 'react';

interface SMChannelDataForSMChannelSelectorForDataKeyProperties {
...
const SMChannelDataForSMChannelSelectorForDataKey = ({
  height,
  dataKey,
  onChange,
  selectedItems,
  name
}: SMChannelDataForSMChannelSelectorForDataKeyProperties) => {
  ...
  const addOrRemoveTemplate = useCallback(
    (data: any) => {
      ...
      if (found) {
        if (name !== undefined) {
          toolTip = 'Remove Channel From "' + name + '"';
        } else {
...
export default memo(SMChannelDataForSMChannelSelectorForDataKey);
