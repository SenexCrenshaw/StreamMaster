...
import {
  AuthenticationType,
  StreamingProxyTypes,
} from '@/lib/common/streammaster_enums'
...
export const SettingsEditor = () => {
  ...
  const adminUserNameError = React.useMemo((): string | undefined => {
    if (
      newData.authenticationMethod === AuthenticationType.Forms &&
      newData.adminUserName === ''
    )
        ...
        <>
          {getLine(
            ...
            <Dropdown
              className="withpadding w-full text-left"
              onChange={(e) =>
                setNewData({ ...newData, [field]: parseInt(e.target.value) })
              }
              options={options}
              placeholder={label}
              value={getRecordString(field)}
      ...
      <ScrollPanel style={{ height: 'calc(100vh - 18px)', width: '100%' }}>
        ...
        <div className="justify-content-between align-items-center">
          ...
          <Fieldset className="mt-4 pt-10" legend={GetMessage('general')}>
            ...
            {newData.enableSSL === true && (
              <>
                {getInputTextLine(
                  'sslCertPath',
                  GetMessage('changesServiceRestart'),
                )}
                {getPasswordLine(
                  'sslCertPassword',
                  GetMessage('changesServiceRestart'),
          ...
          <Fieldset
            ...
            legend={GetMessage('authentication')}
          >
              ...
              <div className="flex col-3 m-0 p-0 debug">
                <Button
                  disabled={
                    ...
                    (setting.authenticationType as number) === 0
                  }
                  icon="pi pi-check"
          ...
          <Fieldset className="mt-4 pt-10" legend={GetMessage('filesEPGM3U')}>
            ...
            <Fieldset
              className="mt-4 pt-10"
              collapsed
              legend={GetMessage('nameregexSettings')}
              toggleable
            >
              <SettingsNameRegexDataSelector
                data={settingsQuery.data?.nameRegex}
              />
            </Fieldset>
            <Fieldset
              className="mt-4 pt-10"
              collapsed
              legend={GetMessage('m3uSettings')}
              toggleable
            >
