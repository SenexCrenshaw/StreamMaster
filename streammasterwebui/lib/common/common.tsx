...
export const hasValidAdditionalProps = (
  additionalFilterProps: AdditionalFilterProps | undefined,
) => {
  return additionalFilterProps?.values
}

export function getChannelGroupMenuItem(
  colorIndex: string | undefined,
  toDisplay: string,
): React.ReactNode {
  return (
    <div className="gap-2">
      <div>
        <i
          className="pi pi-circle-fill pr-2"
...
export function areFilterMetaEqual(
  a: DataTableFilterMeta,
  b: DataTableFilterMeta,
): boolean {
...
export function areAdditionalFilterPropsEqual(
  a: AdditionalFilterProps | undefined,
  b: AdditionalFilterProps | undefined,
): boolean {
...
export function areDataTableFilterMetaDataEqual(
  a: SMDataTableFilterMetaData,
  b: SMDataTableFilterMetaData,
): boolean {
...
export function areDataTableFilterMetaDatasEqual(
  arr1: SMDataTableFilterMetaData[],
  arr2: SMDataTableFilterMetaData[],
): boolean {
...
export function areGetApiArgsEqual(
  obj1?: GetApiArg,
  obj2?: GetApiArg,
): boolean {
...
export function areIconFileDtosEqual(
  array1: IconFileDto[],
  array2: IconFileDto[],
): boolean {
...
export const doSetsContainSameIds = (
  set1: Set<number | string>,
  set2: Set<number | string>,
): boolean => {
...
export function isChildVideoStreamDto(
  value: unknown,
): value is ChildVideoStreamDto {
...
export function areVideoStreamsEqual(
  ...
  for (let i = 0; i < streams1.length; i++) {
    ...
    if (
      isChildVideoStreamDto(streams1[i]) &&
      isChildVideoStreamDto(streams2[i])
    ) {
      if (
        (streams1[i] as ChildVideoStreamDto).rank !==
        (streams2[i] as ChildVideoStreamDto).rank
      ) {
        return false
      }
    }
...
function getApiUrl(path: SMFileTypes, originalUrl: string): string {
  return `${isDev ? baseHostURL : ''}/api/files/${path}/${encodeURIComponent(
    originalUrl,
  )}`
}
...
export function getIconUrl(
  iconOriginalSource: string | null | undefined,
  defaultIcon: string,
  cacheIcon: boolean,
): string {
...
export const hasColumns = (columns?: ColumnMeta[]) =>
...
export const ExportComponent: React.FC<{ readonly exportCSV: any }> = ({
  exportCSV,
