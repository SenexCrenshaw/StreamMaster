...
const selectedChannelGroupsSlice = createSlice({
  ...
  reducers: {
    setselectedChannelGroupsInternal: (
      state,
      action: PayloadAction<selectedChannelGroupsSlicePayload>,
    ) => {
...
const selectVideoStreamsState = (state: RootState) =>
  state.selectedChannelGroups
...
export const { setselectedChannelGroupsInternal } =
  selectedChannelGroupsSlice.actions
