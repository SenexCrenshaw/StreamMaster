...
const selectedStreamGroupSlice = createSlice({
  ...
  reducers: {
    setSelectedStreamGroupInternal: (
      state,
      action: PayloadAction<SetSelectedStreamGroupSlicePayload>,
    ) => {
...
export const selectedStreamGroup = (state: RootState, typename: number) =>
  state.selectedStreamGroup[typename]
export const { setSelectedStreamGroupInternal } =
  selectedStreamGroupSlice.actions
