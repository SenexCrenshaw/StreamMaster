...
import { fetchGetLogo } from '@lib/smAPI/Logos/GetLogoFetch';
...
const getLogoSlice = createSlice({
  ...
  reducers: {
    ...
    clearByTag: (state, action: PayloadAction<{ tag: string }>) => {
      ...
      Logger.debug('GetLogo clearByTag');
    },
    ...
    setIsForced: (state, action: PayloadAction<{ force: boolean }>) => {
      ...
      Logger.debug('GetLogo  setIsForced ', force);
    },
    setIsLoading: (state, action: PayloadAction<{ param: string; isLoading: boolean }>) => {
      ...
      Logger.debug('GetLogo setIsLoading ', action.payload.isLoading);
    }
  },

  extraReducers: (builder) => {
    builder
      .addCase(fetchGetLogo.pending, (state, action) => {
      ...
      .addCase(fetchGetLogo.fulfilled, (state, action) => {
        if (action.payload) {
      ...
      .addCase(fetchGetLogo.rejected, (state, action) => {
        const paramString = JSON.stringify(action.meta.arg);
...
export const { clear, clearByTag, setIsLoading, setIsForced, setField } = getLogoSlice.actions;
