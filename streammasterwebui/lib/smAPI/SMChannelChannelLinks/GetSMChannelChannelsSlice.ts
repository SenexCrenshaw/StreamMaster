...
import { fetchGetSMChannelChannels } from '@lib/smAPI/SMChannelChannelLinks/GetSMChannelChannelsFetch';
...
const getSMChannelChannelsSlice = createSlice({
  initialState,
  name: 'GetSMChannelChannels',
  reducers: {
    clear: (state) => {
      state = initialState;
      Logger.debug('GetSMChannelChannels clear');
    },
    ...
    clearByTag: (state, action: PayloadAction<{ tag: string }>) => {
      ...
      Logger.debug('GetSMChannelChannels clearByTag');
    },
    ...
    setField: (state, action: PayloadAction<{ fieldData: FieldData }>) => {
      ...
      Logger.debug('GetSMChannelChannels setField');
    },
    setIsForced: (state, action: PayloadAction<{ force: boolean }>) => {
      ...
      Logger.debug('GetSMChannelChannels  setIsForced ', force);
    },
    setIsLoading: (state, action: PayloadAction<{ param: string; isLoading: boolean }>) => {
      ...
      Logger.debug('GetSMChannelChannels setIsLoading ', action.payload.isLoading);
    }
  },

  extraReducers: (builder) => {
    builder
      .addCase(fetchGetSMChannelChannels.pending, (state, action) => {
      ...
      .addCase(fetchGetSMChannelChannels.fulfilled, (state, action) => {
        if (action.payload) {
      ...
      .addCase(fetchGetSMChannelChannels.rejected, (state, action) => {
        const paramString = JSON.stringify(action.meta.arg);
...
export const { clear, clearByTag, setIsLoading, setIsForced, setField } = getSMChannelChannelsSlice.actions;
export default getSMChannelChannelsSlice.reducer;
