...
import {FieldData, LineupPreviewChannel } from '@lib/smAPI/smapiTypes';
import { fetchGetLineupPreviewChannel } from '@lib/smAPI/SchedulesDirect/GetLineupPreviewChannelFetch';


interface QueryState {
  data: Record<string, LineupPreviewChannel[] | undefined>;
...
const getLineupPreviewChannelSlice = createSlice({
  initialState,
  name: 'GetLineupPreviewChannel',
  reducers: {
    clear: (state) => {
      state = initialState;
      Logger.debug('GetLineupPreviewChannel clear');
    },
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchGetLineupPreviewChannel.pending, (state, action) => {
      ...
      .addCase(fetchGetLineupPreviewChannel.fulfilled, (state, action) => {
        if (action.payload) {
      ...
      .addCase(fetchGetLineupPreviewChannel.rejected, (state, action) => {
        const paramString = JSON.stringify(action.meta.arg);
...
export const { clear, clearByTag, setIsLoading, setIsForced, setField } = getLineupPreviewChannelSlice.actions;
export default getLineupPreviewChannelSlice.reducer;
