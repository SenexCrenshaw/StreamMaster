...
import { fetchGetVideoInfo } from '@lib/smAPI/Statistics/GetVideoInfoFetch';
...
const getVideoInfoSlice = createSlice({
  ...
  reducers: {
    clear: (state) => {
      state = initialState;
      Logger.debug('GetVideoInfo clear');
    },
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchGetVideoInfo.pending, (state, action) => {
      ...
      .addCase(fetchGetVideoInfo.fulfilled, (state, action) => {
        if (action.payload) {
      ...
      .addCase(fetchGetVideoInfo.rejected, (state, action) => {
        const paramString = JSON.stringify(action.meta.arg);
...
export const { clear, clearByTag, setIsLoading, setIsForced, setField } = getVideoInfoSlice.actions;
