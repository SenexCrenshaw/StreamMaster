...
const SchedulesDirectCountrySelector = (props: SchedulesDirectCountrySelectorProps) => {
  ...
  const options = React.useMemo(() => {
    ...
    if (getCountriesQuery.data["North America"]) {
      countries.push(
        ...getCountriesQuery.data["North America"]
          ...
          .map((c) => { return { label: c.fullName, value: c.shortName } })
      );
    }

    if (getCountriesQuery.data.Europe) {
      countries.push(
        ...getCountriesQuery.data.Europe
          .filter((c) => c?.shortName !== undefined && c.shortName.trim() !== '')
          .map((c) => { return { label: c.fullName, value: c.shortName } })
      );
    }
    ...
    if (getCountriesQuery.data.Caribbean) {
      countries.push(
        ...getCountriesQuery.data.Caribbean
          .filter((c) => c?.shortName !== undefined && c.shortName.trim() !== '')
          .map((c) => { return { label: c.fullName, value: c.shortName } })
      );
    }
    ...
    if (getCountriesQuery.data.Oceania) {
      countries.push(
        ...getCountriesQuery.data.Oceania
          .filter((c) => c?.shortName !== undefined && c.shortName.trim() !== '')
          .map((c) => { return { label: c.shortName + '-' + c.fullName, value: c.shortName } })
      );
    }
